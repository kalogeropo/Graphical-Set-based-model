...
def pruneGraphbyWeight(aMatrix,termlist):
    ...
    # test1 +30% file name newfilep30h100.dat slight imporvement 1.2*1.3 = 1.56 or 156% average
    # test2 -20% file name newfilem20h100.dat worse  96 % * average
    # test3  average * 80%
    # test4  average * 60%
    # test5  average * 140%
    # test6  average * 40%
    # Test7  average * 20%
    # test8  average * 10%
    # testing large scale h (100(*),300(*),300e(*),500(*),700(*)) indexes created with 1.2
    # Paratirw oti me parametro pruning 1,2*average gia h=300 apantame me kalitero Av precision gia 45
    # erwtimata
    # h300 , 120% * average
    # h300 , 60% *average
    # h300 , 40%
    # h700 , 60%<--- warning:large scalars -> error math domain error on log
    # h500e ,60%  average test 1 (warning large scalars no error)
    # h300e , 60% *average
    # h300e , 80% *average
    # h300e , 100% average
    # h400e , 60%  aver
    # h400e , 120% aver
    # h600ee , 80%  aver
    # h600ee , 60%  aver failed! math domain error
    # h500ee , 60% <---best!
    # testing density 0*6 average !!!!!ISSUES 1 OR 2 CORES
    # testing density 0*8 average !!!!!ISSUES 1 OR 2 CORES
    # TESTING density method  on average
    # h =500ee
    # testing all on average
    # testing all on .8 average
    # testing all on .6 averaga
    # testing all on .4 average

    # h = 400ee
    #testing on .6 :1,3,4
    #testing on .8 :1 ,3,4
    #testing on 1.0 :1 3 ,4
    # testing CR on edge values (.4 av)
    # testing all on 1.3 av 1

    #testing review
    #with no preproccess elements
    #all @400ee 0,4 av (tested twice result !!!!!!)
    #all @400ee 0.6 av
    #all @400ee 0.8 av

    #all @500ee 0.8av

    #Preproccessed files - stopwords - num_data
    # all @400ee 0,6av
    # all @400ee 0,8av
    # all @400ee 0.4av
    # all @400ee 1.2av
    ...
    #test akraiees times 100 kai 800 @08av (@1000 error)
    # 800 08av
    # 100 08av ---> pretty similar results why
    # testing for some error using 1 08 av if results differ al lot from doukas we have an issue
    ...
    #new set
    #n 200 0.6 1 2 3 4 similar

    #n 400 all

    #n 700 0.8 1 2 3 4 !!!!!!highest result -> rip highscore
    ...
    #n 2000 0.8 1 2 3 4
    #n 2000 0.2, 0.3, 0.4, 0.6(to redo)
    gr = nx.Graph()
...
#here we write the results to excel in order for automation with perpose of further editing
        ...
        writer = pd.ExcelWriter("/home/nik/PycharmProjects/k-core-decomp/results.xlsx" , engine="openpyxl" , mode="a")
        writer.book = load_workbook("/home/nik/PycharmProjects/k-core-decomp/results.xlsx" )
